...
int String32::size() const {
		return m_data.size();
}

int String32::at(int index) const {
		return m_data.at(index);
}

void String32::push_back(int value) {
...
std::string Convert::encodeToUTF8(const String32& utf32String) {
		...
		for(int index = 0 ; index < utf32String.size(); ++index) {
            int i = utf32String.at(index);
			// 0xxxxxxx
			if(i < 0x80) {
